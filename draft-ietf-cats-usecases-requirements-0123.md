...
R2: MUST set up a frequency for updating the computing status of different service instances to guarantee effective traffic steering.

R3: MUST provide a method to determine the availability of a
service instance.
...
R4: MUST agree on using metrics that are oriented towards compute
...
R5: A model of the compute and network resources MUST be defined. Such a model MUST characterize how metrics are abstracted out from the compute and network resources. We refer to this model as the Resource Model.

R6: The Resource Model MUST be implementable in an interoperable manner. That is, independent implementations of the Resource Model must be interoperable.

R7: The Resource Model MUST be executable in a scalable manner. That is, an agent implementing the Resource Model MUST be able to execute it at the required time scale and at an affordable cost (e.g., memory footprint, energy, etc.).

R8: The Resource Model MUST be useful. That is, the metrics that an agent can obtain by executing the Resource Model must be useful to make node and path selection decisions.
...
R9: There MUST set up metric information that can be understood by CATS components. For metrics that CATS components do not understand or support, CATS components will ignore them.

R10: The computing metrics in CATS MUST be simple, that is distributing metrics and selecting path based on these metrics will not cause routing loops and route oscillation. 
...
R11: MUST provide mechanisms for metric collection.
R12: MUST declare the entity that collect metrics.
...
R13: SHOULD provide mechanisms to aggregate the metrics.
...
R14: MUST provide mechanisms to distribute the metrics.
...
R15: MUST be clear of the update frequency of CATS metrics and its corresponding distribution method.

Sometimes, a metric that is chosen is not accurate for service instance selection, in such case, a desirable system

R16: SHOULD provide mechanisms to assess selection accuracy and re-select metrics if the selection result is not accurate.
...
R15: Instance affinity MUST be maintained when the transaction is stateful.
...
R16: Instance affinity MUST be maintained for service requests or transactions that belong to the same flow.
...
R24: When making service decisions, the security status of computing resources SHOULD be taken into consideration.
